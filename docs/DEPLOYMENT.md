...
Hospede o frontend estático com fallback de SPA para `index.html`. Aplique os headers de `docs/SECURITY.md` no CDN. Configure apenas `VITE_SUPABASE_URL`, `VITE_SUPABASE_PUBLISHABLE_KEY`, `VITE_APP_ENV` e a site key pública do Turnstile no build. Secrets ficam no cofre do provedor ou `supabase secrets`.
